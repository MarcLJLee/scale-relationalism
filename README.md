...
> 물리 법칙 자체는 절대적이지만, **어떤 거동이 관측되는가**는 관측의 **에너지 스케일**과
> 대상의 특징 에너지 사이의 **비율**이 정한다는 가설을, 최소 검증가능한 벽돌로 시험하는 프로젝트.
...
아니라 **관측의 에너지 스케일과 대상의 특징 에너지 사이의 비율**이 정한다고 제안한다.
"오직 무차원 비율만 실재한다"(버킹엄 π)는 이를 받치는 **도구**이고, 상수·시공간 자체가 더
근본적 구조에서 창발한다는 **강한 판본은 미확정 베팅**이다. (자세한 공준은 `docs/outline.html`.)
...
> **중간 정산이 궁금하면 → [FINDINGS.md](FINDINGS.md)** (방점 #1: 아홉 벽돌 / #2: 물리의
> 개념·수학 바닥 / #3: 양자·고전 경계 시리즈 — 어디서 통하고 어디서 깨지는지 한눈에).
